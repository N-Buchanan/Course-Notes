...
\example 
$A =\{1,11\}, B=\{00,000\}, w(\sigma) = k | \simga = \sigma_1 \dots \sigma_k$ The generating series are: $\Phi_A(x) = x+x^2, \Phi_B(x) = x^2+x^3$. For $AB, w(aab) = w(a) + w(b)$
...
\begin{defn}
	$AB$ is \textbf{ambiguous} if $\exists$ distinct pairs $(a_1,b_1),(a_2,b_2) \in A \times B$ such that $a_1b_1 = a_2b_2$. Otherwise, $AB$ is \textbf{unambiguous}. In other wrods, each string is uniquely generated.
\end{defn}

When $AB$ is ambiguous, $\Phi_{A \times B}(x) \ne \Phi_{AB}(x)$. An ambiguous string appears in $A \times B$ multiple timesbut only one time in $AB$ ($\exists i | [x^i]\Phi_{A \times B}(x) > [x^i]\Phi_{AB}(x)$)
